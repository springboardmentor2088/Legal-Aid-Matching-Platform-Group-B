...
const NgoDashboard = () => {
    ...
    const handleProfileChange = (e) => {
        ...
        if (['phone', 'phoneNumber', 'contactPhone', 'organizationPhone'].includes(name)) {
    ...
    const handleUpdateProfile = async () => {
        ...
        if (phone && !validatePhone(phone)) {
            alert("Main Phone number must be a valid 10-digit Indian number starting with 6-9.");
            return;
        }
        if (contactPhone && !validatePhone(contactPhone)) {
            alert("Contact Person Phone must be a valid 10-digit Indian number starting with 6-9.");
            return;
        }
        if (orgPhone && !validatePhone(orgPhone)) {
            alert("Organization Phone must be a valid 10-digit Indian number starting with 6-9.");
            return;
        }
        ...
        try {
            ...
            const payload = {
                ...
                registrationYear: editForm.registrationYear ? parseInt(editForm.registrationYear) : null,
                maxProBonoCases: (editForm.maxProBonoCases !== null && editForm.maxProBonoCases !== '' && editForm.maxProBonoCases !== undefined) ? parseInt(editForm.maxProBonoCases) : null,
                ...
                proBonoCommitment: editForm.proBonoCommitment === 'true' || editForm.proBonoCommitment === true,
                ...
                serviceAreas: Array.isArray(editForm.serviceAreas)
                    ...
                    : (typeof editForm.serviceAreas === 'string' ? editForm.serviceAreas.split(',').map(s => s.trim()) : []),
            };
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                {user?.role === 'NGO' && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    value={isEditing ? editForm.organizationPhone || '' : profile?.organizationPhone || ''}
                                                    onChange={handleProfileChange}
                                                    disabled={!isEditing}
                                                    className={`w-full pl-20 p-2 border rounded-lg ${isEditing ? 'border-primary bg-white focus:ring-2 ring-primary/20' : 'bg-gray-50 border-gray-200 text-gray-600'}`}
                                                />
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                ...
                                                value={isEditing ? editForm.organizationEmail || '' : profile?.organizationEmail || ''}
                                                onChange={handleProfileChange}
                                                disabled={!isEditing}
                                                className={`w-full p-2 border rounded-lg ${isEditing ? 'border-primary bg-white focus:ring-2 ring-primary/20' : 'bg-gray-50 border-gray-200 text-gray-600'}`}
                                            />
                                        ...
                                        <div className="md:col-span-2">
                                            <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wide mb-3 mt-4 border-b pb-1">Contact Person Details</h3>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                value={isEditing ? editForm.contactPersonDesignation || '' : profile?.contactPersonDesignation || ''}
                                                onChange={handleProfileChange}
                                                disabled={!isEditing}
                                                className={`w-full p-2 border rounded-lg ${isEditing ? 'border-primary bg-white focus:ring-2 ring-primary/20' : 'bg-gray-50 border-gray-200 text-gray-600'}`}
                                            />
                                        ...
                                        <div className="md:col-span-2">
                                            <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wide mb-3 mt-4 border-b pb-1">Operational Details</h3>
                                        </div>
                                        ...
                                        <div className="md:col-span-2">
                                            ...
                                            <input
                                                type="text"
                                                name="serviceAreas"
                                                value={isEditing ? (Array.isArray(editForm.serviceAreas) ? editForm.serviceAreas.join(', ') : editForm.serviceAreas || '') : (Array.isArray(profile?.serviceAreas) ? profile.serviceAreas.join(', ') : profile?.serviceAreas || '')}
                                                onChange={(e) => {
                                                    const val = e.target.value.split(',').map(s => s.trim());
                                                    setEditForm(prev => ({ ...prev, serviceAreas: val }));
                                                }}
                                                disabled={!isEditing}
                                                placeholder="Legal Aid, Women's Rights, etc."
                                                className={`w-full p-2 border rounded-lg ${isEditing ? 'border-primary bg-white focus:ring-2 ring-primary/20' : 'bg-gray-50 border-gray-200 text-gray-600'}`}
                                            />
                                        </div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            <label className="flex items-center gap-2 cursor-pointer">
                                                <input
                                                    ...
                                                    checked={isEditing ? editForm.proBonoCommitment || false : profile?.proBonoCommitment || false}
                                                    onChange={(e) => setEditForm(prev => ({ ...prev, proBonoCommitment: e.target.checked }))}
                                                    disabled={!isEditing}
                                                    className="w-4 h-4 text-primary rounded focus:ring-primary"
                                                />
                                                <span className="text-sm font-medium text-gray-700">Pro-Bono Commitment</span>
                                            </label>
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Max Monthly Cases</label>
                                            <input
                                                type="number"
                                                name="maxProBonoCases"
                                                value={isEditing ? editForm.maxProBonoCases || '' : profile?.maxProBonoCases || ''}
                                                onChange={handleProfileChange}
                                                disabled={!isEditing}
                                                className={`w-full p-2 border rounded-lg ${isEditing ? 'border-primary bg-white focus:ring-2 ring-primary/20' : 'bg-gray-50 border-gray-200 text-gray-600'}`}
                                            />
